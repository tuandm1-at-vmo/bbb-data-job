...
from .entity import SEComponent


logger = get_logger(__package__)

COLLECTION = 'se_component'
TABLE = 'bbb_data_source.se_component'
...
def fetch_untransfered_documents_by_page(page: int, size: int, pool: MongoPool):
    ...
    with conn:
        ...
        docs = conn.fetch(collection=COLLECTION,
                          ...
                          skip=(page*size),
                          limit=size,
                          doc_type=SEComponent)
        logger.debug('Total documents fetched [page=%s,size=%s]: %s', page, size, len(docs))
        return docs


def insert_records_from_untransfered_documents(docs: List[SEComponent], pool: MssqlPool):
...
def mark_transfered_documents(docs: List[SEComponent], pool: MongoPool):
    if len(docs) == 0: return []
    conn = pool.get_connection(timeout_ms=30000, connection_type=MongoConnection)
    with conn:
        query_filter = {
...
def start(sql: MssqlPool, mongo: MongoPool) -> tuple[List[SEComponent]]:
