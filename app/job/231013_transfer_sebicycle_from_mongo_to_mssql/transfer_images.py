...
from .entity import SEImage


logger = get_logger(__package__)

COLLECTION = 'se_image'
TABLE = 'bbb_data_source.se_image'
...
def insert_records_from_untransfered_documents(docs: List[SEImage], pool: MssqlPool):
...
def mark_transfered_documents(docs: List[SEImage], pool: MongoPool):
    if len(docs) == 0: return []
    conn = pool.get_connection(timeout_ms=30000, connection_type=MongoConnection)
    with conn:
        query_filter = {
...
def start(sql: MssqlPool, mongo: MongoPool) -> tuple[List[SEImage]]:
